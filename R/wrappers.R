...
#' to have unit norm.
#'
#' @return A list of length \code{n} of \code{igraph} graph objects.
#'
#' @export
genet <- function(n, assignments, cards, dc = TRUE, target_centrality,
                  target_degree = NA, verbose = FALSE,
                  return_root = FALSE) {
  ...
  if (!dc) { # rescale root to comply with target average degree
    ...
    avg_expected_degree <-
      sum(cards * compute_expected_degrees(root, interactions, target_centrality)) / sum(cards)
    root <- root * sqrt(target_degree / avg_expected_degree)
  }
  if (return_root) {
    out <- list(
      "root" = root,
      "edge_lists" = rBKN_wrapper(n, cards, H, root, verbose, F)
    )
  } else {
